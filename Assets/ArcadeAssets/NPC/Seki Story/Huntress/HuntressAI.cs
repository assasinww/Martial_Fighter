...
public class HuntressAI : MonoBehaviour
{
    private Rigidbody2D rb;
    public Transform PreSpear;
    public Animator animator;
    private int DecisionNumber;
    private GameObject Opponent;
    private FaceOpponent Flipped;
    private float Cooldown1; private float Cooldown2; private float Cooldown3; private float DashCooldown;
    private float movespeed; public bool P2Blocking;
    private float range;
    private bool Attacking = false; private bool Attacked = false;private bool RangedAttack = false;
    ...
    void Update()
    {
        ...
        if (ControlsActive.ControlActive == true && ControlsActive.MatchOver == false)
        {
            ...
            if (Cooldown1 <= 0 && Cooldown2 <= 0 && Cooldown3 <= 0)
            {
                Attacking = false;
            }
            ...
            //Ranged attack bool causes Huntress only (due to her being only character with a ranged attack) to move backwards to spear throwing range
            if (DecisionNumber == 8 && Cooldown3 <= 0 && range > 6 && ControlsActive.Map1.active == true)
            {
                RangedAttack = true;
            }
            if (DecisionNumber == 8 && (Pos.x - Opponent.transform.position.x) < -8 || (Pos.x - Opponent.transform.position.x) > 8 && Cooldown3 <= 0 && Grab.P1Grabbing == false && ControlsActive.Map1.active == true)
            {
                Attacking = true;
                RangedAttack = false;
                Invoke("Throw", 1f);
    ...
    private void FixedUpdate()
    {
        if (ControlsActive.ControlActive == true && ControlsActive.MatchOver == false)
        {
            ...
            {
                if (Attacked == false && Flipped.flip == false && Attacking == false && range > 2.7f && RangedAttack == false && Grab.P1Grabbing == false)
                {
                    movespeed = 1.4f;
                    animator.SetTrigger("IsRunning1");
                    rb.AddForce(new Vector2(-1.5f * movespeed, 0), ForceMode2D.Impulse);
                }
                else if (Attacked == false && Flipped.flip == true && Attacking == false && range > 2.7f && RangedAttack == false && Grab.P1Grabbing == false)
                {
                    movespeed = 1.4f;
                    animator.SetTrigger("IsRunning1");
                    rb.AddForce(new Vector2(1.5f * movespeed, 0), ForceMode2D.Impulse);
                }
                else if (range <= 2.7f || RangedAttack == true)
                {
                    animator.ResetTrigger("IsRunning1");

                }
            }
            //moves the ai to ranged attack distance
            if (RangedAttack == true && ControlsActive.Map1.active == true)
            {
                movespeed = 0.9f;
                animator.SetTrigger("IsRunning2");
                rb.AddForce(new Vector2(1.5f * movespeed, 0), ForceMode2D.Impulse);
            }
            else if (RangedAttack == false && DecisionNumber != 2 && DecisionNumber != 4 && DecisionNumber != 6 && ControlsActive.Map1.active == true)
            {
                animator.ResetTrigger("IsRunning2");
            }
